...
export function SentenceBuilderDrill() {
  const questions = useMemo(() => shuffle(sentenceBuilderQuestions), []);
  const [index, setIndex] = useState(0);
  const [selected, setSelected] = useState<number | null>(null);
  const [score, setScore] = useState({ correct: 0, wrong: 0 });
  const [wrongOnes, setWrongOnes] = useState<SentenceBuilderQuestion[]>([]);
                ...
                {wrongOnes.map((wq, i) => (
                  ...
                  >
                    <div className="flex items-center gap-2 mb-1">
                      <span
                        className={`text-xs font-medium px-2 py-0.5 rounded ${patternColors[wq.pattern]}`}
                      >
                        {patternLabels[wq.pattern]}
                      </span>
                    </div>
                    <p className="text-sm text-gray-800">
                      {wq.sentence.replace(
                        "___",
                        `**${wq.options[wq.correctIndex]}**`
                      )}
                    </p>
  ...
  return (
    <div className="max-w-2xl mx-auto p-6">
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-6">
        {/* Pattern tag - shown after answering */}
        <div className="mb-2 h-6">
          {selected !== null && (
            <span
              className={`text-xs font-medium px-2 py-0.5 rounded ${patternColors[q.pattern]}`}
            >
              {patternLabels[q.pattern]}
            </span>
          )}
        </div>
        ...
        <p className="text-lg text-gray-800 mb-6 leading-relaxed">
          {q.sentence.split("___").map((part, i, arr) => (
            <React.Fragment key={i}>
              {part}
              {i < arr.length - 1 && (
                <span className="inline-block min-w-[140px] border-b-2 border-gray-300 mx-1 text-center font-semibold">
        ...
        <div className="grid grid-cols-2 gap-3 mb-4">
          {q.options.map((opt, i) => {
            let cls =
              "border rounded-lg px-4 py-3 text-left transition-all font-medium ";
            if (selected === null) {
              cls +=
